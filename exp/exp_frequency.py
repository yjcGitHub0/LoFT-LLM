...
class Exp_Frequency(Exp_Basic):
    def __init__(self, args, ft=False, path=False):
        ...
        super(Exp_Frequency, self).__init__(args)
    ...
    def vali(self, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                batch_y = torch.fft.rfft(batch_y, dim=1, norm='forward').permute(0, 2, 1)             # x: [B, C, N, P] -> [B, C, N, P // 2 + 1]
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                batch_y = torch.fft.rfft(batch_y, dim=1, norm='forward').permute(0, 2, 1)
                # FALoss
                loss = torch.sum(torch.sum(torch.sum(torch.abs(outputs - batch_y)))) / (outputs.shape[1] * outputs.shape[0])
